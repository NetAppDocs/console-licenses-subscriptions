...
//The contents of this page are reused in the CVO doc site. As a result, any links from this page to other pages must use absolute URLs so that the links resolve from the Console doc site.
...
| The capacity actually billed for the SVM. Each SVM has a minimum billable capacity (4 TiB by default, though this can vary by account). If the SVM's provisioned capacity is at or below that minimum, this value is rounded up to the minimum; otherwise it equals the provisioned capacity.
...
| Workload type
| Indicates whether the SVM's volumes are *Primary* (read/write), actively used for production data, or *Secondary* (DP), data-protection destination volumes such as SnapMirror replicas used for backup and replication.
