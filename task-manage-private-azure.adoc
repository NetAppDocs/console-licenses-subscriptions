...
Accepting and activating the offer multiple times can result in double charging.  
====

.Steps
. Select the private offer link provided by your NetApp sales team and sign in.  
  If no link is available, sign in directly to the Azure portal.  

. Search or select *Marketplace* from the Azure services.  

. On the Marketplace page, select *Private offer management* under the *Management* section.  

. Select *View + accept* to review the details of the private offer.  
...
. Confirm and select *Accept private offer*.  

. Select *Purchase* to complete the purchasing process.  
+
[NOTE]
====
...
. Select *Review and subscribe*, validate the details, and then select *Subscribe*.  

. When the subscription is successfully created, select *Configure account now*. This redirects you to the NetApp Console to complete configuration.  
+
[NOTE]
====
Azure-side automation can take between 30 seconds and 5 minutes to complete.  
If the page was closed before you selected *Configure account now*, return to the service in the Azure portal.  
...
.. Optionally, enable *Replace existing subscription* to automatically replace an existing Marketplace subscription in one Console organization.  
+
[NOTE]
====
...
For more information on marketplace subscriptions and direct licenses billing, see link:concept-billing-preferences.html[Learn about billing preferences for licenses and subscriptions].